...
static constexpr float LAND_VEL_ABS_THRESH_MPS = 0.35f;
static constexpr float LAND_ALT_RANGE_M = 0.6f;
static constexpr uint32_t LAND_STABLE_MS = 4000;
...
float computeVerticalSpeed(float h_now, uint32_t now_ms) {
  ...
  uint32_t dt_ms = now_ms - lastVelMs;
  if (dt_ms < 5) return vel_filt;

  float dt = dt_ms / 1000.0f;
  float v = (h_now - lastH) / dt;

  // Low-pass velocity for robust detection
  const float VEL_ALPHA = 0.25f;
  vel_filt = VEL_ALPHA * v + (1.0f - VEL_ALPHA) * vel_filt;
